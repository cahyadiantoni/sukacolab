...
class UrProjectDetailViewModel(
    ...
    fun getUserJoin(projectId: String) = viewModelScope.launch {
        projectRepo.getUserJoin(projectId)
            .onStart {
                responseUserJoin.value = UserJoinUiState.Loading
            }.catch {
                responseUserJoin.value = UserJoinUiState.Failure(it)
            }.collect {
                responseUserJoin.value = UserJoinUiState.Success(it)
            }
    }
